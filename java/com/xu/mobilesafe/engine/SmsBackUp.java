...
* 回调方法的经典例子，观察者设计模式
*
* 短信备份，需要什么在调用方法的时候穿过来
*
* 备份显示进度有2种方法，一种是进度条的方法，还有一种是对话框的形式，在这里先处理好，由调用者来决定用对话框还是进度条
* */
public class SmsBackUp {
	...
	public static void backup(Context ctx,String path,CallBack callBack) {
		...
		try {
			...
			//参数：访问内容提供者的地址，获取需要的字段，查询条件null，
			...
			//DTD(xml规范)，是否独立存在的xml。不依赖于任何dtd等等
			newSerializer.startDocument("utf-8", true);
			//设置一个名空间，不需要就null，起点的名称
...
//			pd.setMax(cursor.getCount());
			...
			while(cursor.moveToNext()){
				newSerializer.startTag(null, "sms");//开始
				
				newSerializer.startTag(null, "address");//开始
				newSerializer.text(cursor.getString(0));
				newSerializer.endTag(null, "address");//结束
				
				newSerializer.startTag(null, "date");//开始
				newSerializer.text(cursor.getString(1));
				newSerializer.endTag(null, "date");//结束
				
				newSerializer.startTag(null, "type");//开始
				newSerializer.text(cursor.getString(2));
				newSerializer.endTag(null, "type");//结束
				
				newSerializer.startTag(null, "body");//开始
				newSerializer.text(cursor.getString(3));
				newSerializer.endTag(null, "body");//结束
				
				newSerializer.endTag(null, "sms");//结束
