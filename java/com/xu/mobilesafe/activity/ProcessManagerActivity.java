...
import com.xu.mobilesafe.db.domain.ProcessInfo;
...
public class ProcessManagerActivity extends Activity implements OnClickListener {
	...
	private void initUI() {
		...
		//监听listView滚动事件，因为悬浮控件需要在滚动到哪里的时候改变
...
//				mCustomerList.remove(processInfo);
		...
		for (ProcessInfo processInfo : killProcessList) {
			...
			//如果用户集合里面包含需要杀死进程的集合里面
			if(mCustomerList.contains(processInfo)){
				//那就移除用户的集合
				mCustomerList.remove(processInfo);
			}
			//如果系统集合里面包含需要杀死进程的集合里面
			...
			//记录释放空间的总大小（每次一杀死进程的大小都要累加起来）
			//processInfo.memSize是当前进程占用的空间大小
